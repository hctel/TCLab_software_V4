...
# Taken from TCLab package (DBR)
# Add load disturbance self.DL (DBR)
...
class TCLab(object):
    ...
    def scan(self):
        ...
        DL = self.DL()  # float(self.receive())        
        return T1, T2, Q1, Q2, DL

    # Define properties for Q1, Q2 and DL
    U1 = property(fget=Q1, fset=Q1, doc="Heater 1 value")
    U2 = property(fget=Q2, fset=Q2, doc="Heater 2 value")
    U3 = property(fget=DL, fset=DL, doc="Heater 1 DL")    
...
class TCLabModel(object):
    ...
    def DL(self, val=None):
        ...
        self.update()
        if val is not None:
            self._DL = int(round(clip(val, 0, 1)))
        return self._DL    
